...
export const MusicEditor: React.FC<MusicEditorProps> = ({
            ...
            <View key={voice.key} style={styles.staffLine}>
              ...
              <View style={styles.staffContent}>
                ...
                <TextInput
                  ...
                  multiline={false}
                  scrollEnabled={false}
                  showSoftInputOnFocus={false} // Empêche le clavier natif
                />
              </View>
            </View>
          );
  ...
  const getMeasureCount = (section: Section) => {
    return Math.max(
      ...
      <View style={styles.compositionHeader}>
        ...
        <View style={styles.compositionMeta}>
          <TextComponent style={styles.metaItem}>
            Temps: {composition.tempo}
          </TextComponent>
          <TextComponent style={styles.metaItem}>
            Gamme: {composition.key}
          </TextComponent>
          <TextComponent style={styles.metaItem}>
            {composition.sections.length} section{composition.sections.length > 1 ? 's' : ''}
          </TextComponent>
        </View>
      </View>
      ...
      >
        {composition.sections.map((section) => (
          <View key={section.id} style={styles.sectionContainer}>
            {/* Titre de section repositionné */}
            {showSectionTitles && (
              <View style={[
                styles.sectionTitleContainer,
                activeSectionId === section.id && styles.activeSectionTitleContainer
              ]}>
                <TextComponent style={styles.sectionTitle}>
                  {section.name}
                </TextComponent>
                <TextComponent style={styles.sectionInfo}>
                  {getTotalNotes(section)} notes • {getMeasureCount(section)} mesures
                </TextComponent>
              </View>
            )}
